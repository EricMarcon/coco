...
- les piquets de la bordure droite sont déplacés à la positiion des piquets de la sosu-parcelle suivante à droite, si l'information est disponible (ex.: les piquets de la sous-parcelle 23 sont replacés à la position de ceux de la sous-parcelle 24).
...
for (subplot in unique(quadrats$subplot)) {
  ...
  # Apply the rotation matrix and add the coordinates of the origin of the plot
  xy <- t(solve(local2utm(subplot, surveyor, subplots)) %*% t(xy_utm))
  # Save the coordinates
  quadrats[is_in_subplot, c("x", "y")] <- xy
}
...
## Projection dans le repère des quadrats réels
...
Les arbres doivent être projetés dans le repère de chacun des quadrats réels.
...
La matrice de projection de chaque quadrat de ses coordonnées de terrain (repère orthonormé, côtés de 10m) dans le repère du quadrat réel est calculée par la fonction `ortho2real()`.
...
La projection du repère orthonormé au repère du quadrat réel utilise la matrice inverse.
...
La forme de référence est le quadrat réel (en bleu), qui est un quadrilatère.
Le repère local du quadrat a été projeté dans celui du quadrat local: le quadrat théorique issu du terrain (en rouge) est un parallélogramme.
...
La valeur minimale de l'abscisse est plus simple parce que les coordonnnées du point bas gauche sont (0,0):
...
$x_M$ et $y_M$ les valeurs maximales assumées pendant les mesures de terrain, ici 10 mètres, projetées dans repère du quadrat réel.

La fonction `trees_interpolated()` interpole la position des arbres dans les limites de chaque quadrat.
Elle projette les arbres dans le repère produit par `ortho2real()`.
La position des arbres est alors interpolée.
