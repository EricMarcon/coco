...
for (subplot_number in unique(quadrats$subplot)) {
  ...
  for (x in unique(quadrats[is_in_subplot, "quadra.nb.X"])) {
    ...
    quadrats_edges %>% 
      add_row(
        ...
        Q_lengthX = quadrats[
          is_in_subplot & 
            (quadrats$quadra.nb.X == x) &
            (quadrats$quadra.nb.Y == up_edge - 1), 
          "Q_lengthX1"
        ],
        x = 0, 
        y = 0
  ...
  for (y in unique(quadrats[is_in_subplot, "quadra.nb.Y"])) {
    # Add the right-edge piquet
    quadrats_edges %>% 
      add_row(
        subplot = subplot_number,
        quadra.nb.X = right_edge,
        quadra.nb.Y = y,
        # Distance to the next piquet is in the previous piquet data
        Q_lengthY = quadrats[
          is_in_subplot & 
            (quadrats$quadra.nb.X == right_edge - 1) &
            (quadrats$quadra.nb.Y == y) , 
          "Q_lengthY1"
        ],
        x = 0, 
        y = 0
...
quadrats %>% 
  ...
  arrange(subplot, quadra.nb.X, quadra.nb.Y) -> quadrats  
```
...
for (i in seq_len(nrow(quadrats))) {
  ...
  if (quadrats[i, "quadra.nb.X"] > 0) {
    # Previous piquet on the left
    is_piquet_left <- 
      # Same subplot
      (quadrats$subplot == quadrats$subplot[i]) &
      # Left column
      (quadrats$quadra.nb.X == quadrats$quadra.nb.X[i] - 1) & 
      # Same row
      (quadrats$quadra.nb.Y == quadrats$quadra.nb.Y[i])
    # Cumulate
    quadrats[i, "x"] <- 
      quadrats[is_piquet_left, "x"] + quadrats[is_piquet_left, "Q_lengthX"]
  }
  # Compute y. Ignore edges
  if (quadrats[i, "quadra.nb.Y"] > 0) {
    # Previous piquet down
    is_piquet_below <- 
      # Same subplot
      (quadrats$subplot == quadrats$subplot[i]) &
      # Same column
      (quadrats$quadra.nb.X == quadrats$quadra.nb.X[i]) & 
      # Row below
      (quadrats$quadra.nb.Y == quadrats$quadra.nb.Y[i] - 1)
    # Cumulate
    quadrats[i, "y"] <- 
