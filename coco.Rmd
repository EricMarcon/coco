...
La matrice de projection de chaque quadrat de ses coordonnées de terrain dans le repère du quadrat réel est calculée par la fonction `ortho2real()`.
...
```{r, echo=FALSE}
knitr::include_graphics("images/interpolation2.png")
```
...
où $(x_0, y_0)$ sont les coordonnées du point avant interpolation et $x_M$ et $y_M$ les valeurs maximales assumées pendant les mesures de terrain, ici 10 mètres.
...
trees_interpolated <- function(
  ...
  ortho2real(x_ul, x_dr, x_dl, y_ul, y_dr, y_dl) %*% 
    ...
    # prepare interpolated colums
    mutate(x = NA, y = NA) -> xy_quadrat
  # Interpolate. Use local coordinates, i.e. down left is (0, 0)
  x_ull <- x_ul - x_dl
  x_url <- x_ur - x_dl
  x_drl <- x_dr - x_dl
  y_ull <- y_ul - y_dl
  y_url <- y_ur - y_dl
  y_drl <- y_dr - y_dl
  for (i in seq_len(nrow(xy_quadrat))) {
    x_max <- x_drl + (xy_quadrat[i, "y_0"] - y_drl) * (x_url - x_drl) / (y_url - y_drl)
    x_min <- xy_quadrat[i, "y_0"] * (x_ull / y_ull)
    xy_quadrat[i, "x"] <- x_min + (xy_quadrat[i, "x_0"] - x_min) * (x_max - x_min) / (x_M)
    y_max <- y_ull + (xy_quadrat[i, "x_0"] - x_url) * (y_ull - y_url) / (x_ull - x_url)
    y_min <- xy_quadrat[i, "x_0"] * (y_drl / x_drl)
    xy_quadrat[i, "y"] <- y_min + (xy_quadrat[i, "y_0"] - y_min) * (y_max - y_min) / (y_M)
  }
  return(xy_quadrat[c("x", "y")])
}
```
